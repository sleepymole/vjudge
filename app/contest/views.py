...
def submit():
    ...
    if datetime.utcnow() < c.start_time:
        ...
        return redirect(
            url_for(".problem", contest_id=contest_id, problem_id=problem_id)
        )
    ...
    if not (
        Problem.query.filter_by(oj_name=oj_name, problem_id=real_pid).first()
    ...
    max_seq = (
        db.session.query(func.max(ContestSubmission.seq))
        .filter_by(contest_id=c.id)
        .first()[0]
        or 0
    )
...
def problem(contest_id, problem_id):
    ...
    if current_user.is_authenticated:
        res = (
            db.session.query(
                ContestSubmission.source_code.label("code"),
                ContestSubmission.language.label("lang"),
            )
            .filter_by(
                user_id=current_user.id,
            ...
            .order_by(ContestSubmission.id.desc())
            .first()
        )
        if res:
...
def problem_list(contest_id):
    ...
    contest = g.contest
    pagination = (
        Problem.query.filter_by(oj_name=contest.clone_name)
        .order_by(Problem.problem_id)
        .paginate(page, per_page=per_page, error_out=False)
    )
    return render_template(
        "contest/problem_list.html",
        contest=contest,
...
def status(contest_id):
    ...
    pagination = (
        ContestSubmission.query.filter_by(**query_args)
        .order_by(ContestSubmission.id.desc())
        .paginate(page, per_page=per_page, error_out=False)
    )
    submissions = []
...
def source_code(contest_id):
    ...
    submission = ContestSubmission.query.filter_by(
        contest_id=contest.id, seq=seq
    ).first()
    if not submission:
        abort(404)
    ...
    if (
        ...
        and (
            datetime.utcnow() < contest.end_time
            or not current_user.can(Permission.MODERATE)
